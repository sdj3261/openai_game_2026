...
| 심사 기준 | 제출 빌드의 근거 |
|---|---|
| **Playability** | 공개 HTTPS 웹 빌드, PC와 모바일 조작, 9개 완성 스테이지, 점수와 진행 저장, 57개 단위 테스트와 정적 빌드 스모크 검사 |
...
> v0.12.0 영상과 게임은 `Z 경비원 도발`, `X 분신 만들기`라는 같은 조작 용어를 사용합니다. `Z 경비원 도발`은 선택 행동이고, 스테이지 2부터는 분신이 필요합니다. 마지막 스테이지는 분신 3개로 문 3개를 열어야 합니다.
...
v0.12.0 자동 검사는 단위 테스트 총 57개와 정적 빌드 스모크 검사로 구성됩니다.
